...
ENV_VAR_REGISTRY: list[EnvVarSpec] = [
    ...
    EnvVarSpec(
        ...
        default="false",
        required=False,
        valid_values=None,  # lenient parsing — see GENERIC_ERRORS
    ),
    EnvVarSpec(
    ...
    EnvVarSpec(
        name="KBM_LEDSAS_DEPLOYMENT_ID",
        description=(
            "Build/commit identifier surfaced verbatim as the "
            "deployment_id field in /health and /ready responses, so an "
            "operator can confirm which build is live behind a probe. "
            "Falls back to KBM_LASTCOMMITID when unset. Omitted from "
            "responses when neither is set or the value is blank."
        ),
        default=None,
        required=False,
    ),
