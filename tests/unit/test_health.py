...
async def started_server():
    ...
    async def _build(
        ...
        server = HealthServer(
            ...
            verbose=verbose,
            emit_deployment_id=emit_deployment_id,
        )
        await server.start()
...
class TestHealthServer:
    ...
    async def test_default_minimal_body_omits_fingerprintable_fields(self, started_server):
        """SEC-5: default (non-verbose) response is minimal — no version, no service, no check names.
    ...
    async def test_default_minimal_body_lists_failing_check_names(self, started_server):
        """SEC-5: on failure the minimal body still reports which checks failed.
...
class TestDeploymentId:
    ...
    @pytest.mark.asyncio
    async def test_deployment_id_suppressed_when_emit_disabled(self, started_server, monkeypatch):
        """When the SDK is NOT the deployment's health surface (emit
        disabled), deployment_id is omitted even if KBM_LASTCOMMITID is set —
        the aggregating component owns it instead."""
        monkeypatch.setenv("KBM_LASTCOMMITID", "abc1234")
        _, url = await started_server(verbose=False, emit_deployment_id=False)
        async with aiohttp.ClientSession() as s:
            async with s.get(f"{url}/health") as r:
                assert await r.json() == {"status": "healthy"}
            async with s.get(f"{url}/ready") as r:
    ...
    @pytest.mark.asyncio
    async def test_deployment_id_suppressed_in_verbose_when_emit_disabled(
        self, started_server, monkeypatch
    ):
        monkeypatch.setenv("KBM_LASTCOMMITID", "abc1234")
        _, url = await started_server(verbose=True, emit_deployment_id=False)
        async with aiohttp.ClientSession() as s:
            async with s.get(f"{url}/health") as r:
                body = await r.json()
