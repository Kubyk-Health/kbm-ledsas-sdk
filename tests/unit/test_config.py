...
class TestSDKConfigDefaults:
    ...
    def test_minimal_config(self):
        ...
        # v0.1.8 health-host default tightened to loopback only.
...
class TestSDKConfigFromEnv:
    ...
    def test_from_env_rabbitmq_url(self):
        """KBM_LEDSAS_RABBITMQ_URL is read on the direct/customer wheel."""
        with patch.dict(
            os.environ,
    ...
    def test_from_env_blob_config(self):
        """Blob configuration from env vars (direct/customer wheel)."""
        with patch.dict(
            os.environ,
            {
...
class TestSDKConfigRepr:
    ...
    def test_repr_full(self):
        """Repr with full config — and no retired dev_mode field."""
        config = SDKConfig(
            service_name="my_service",
            tenant="acme",
            concurrency=20,
        ...
        # The retired dev_mode field must not leak into the repr.
        assert "dev_mode" not in repr_str
